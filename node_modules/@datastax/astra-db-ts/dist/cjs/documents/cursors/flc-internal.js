...
class FLCInternal {
    ...
    async fetchNextPageRaw(extra, tm, opts) {
        const sort = this._serdes.serialize(this._options.sort, ctx_js_1.SerDesTarget.Sort);
        const filter = this._serdes.serialize(this._filter, ctx_js_1.SerDesTarget.Filter);
        const command = {
            [opts.commandName]: {
                filter: filter[0],
